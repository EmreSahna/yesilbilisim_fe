...
const Blog = () => {
    ...
    useEffect(() => {
        ...
        BlogService.getBlogs(currPage-1).then((res) => {
            if (isDone) return;
            setPosts(res.data.blog_responses);
            setTotalPages(res.data.total_pages);

            allImages.push(...res.data.blog_responses.map((item) => item.thumbnail_image));
    ...
    const pageBoxes = () => {
        ...
        for(let i = 1; i < totalPages+1; i++){
            if(i === currPage){
                pageBoxes.push(
                    <button onClick={() => changePage(i)} key={i} className="w-[40px] h-[40px] font-bold border-2 border-main-blue text-white rounded-md flex justify-center items-center bg-main-blue">
                        <span className="text-[20px]">{i}</span>
                    </button>
                );
            }else{
                pageBoxes.push(
                    <button onClick={() => changePage(i)} key={i} className="w-[40px] h-[40px] font-bold border-2 border-main-gray-2 text-main-blue rounded-md flex justify-center items-center">
                        <span className="text-[20px]">{i}</span>
                    </button>
                );
            }
    ...
    return (
        <>
            ...
            <div className="max-w-[1170px] justify-between mx-auto flex my-[50px] max-small:flex-col-reverse max-small:gap-[30px] max-small:mx-[20px]">
                <div className="flex gap-[20px] w-[75%] max-small:w-full flex-wrap">
                    {posts && 
                        posts.map((blog,index) => {
                            return(
                                <article className="w-[47.5%] shadow-card rounded-md h-[300px] max-small:h-[200px]" key={index}>
                                    <a href={"blog/"+blog.url} className="relative group">
                                        <div className="flex absolute top-0 bg-[rgba(8,112,176,.7)] rounded-br-md rounded-tl-md px-1 z-10">
                                            <div className="flex gap-1 items-center text-white">
                                                ...
                                                <p className="text-[16px] font-semibold max-small:text-[12px]">{blog.created_date}</p>    
                                            </div>
                                        </div>
                                        ...
                                        <div className="overflow-hidden h-full group-hover:bg-main-black rounded-md">
                                            <img src={`http://localhost:8080/img/${blog.thumbnail_image}`} className="object-cover w-full h-full rounded-md group-hover:rotate-[5deg] group-hover:opacity-50 group-hover:scale-[1.2] transition" alt="" />
                                        </div>
